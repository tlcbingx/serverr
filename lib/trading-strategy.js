...
class TradingStrategy {
  ...
  update(candle) {
    ...
    // Сброс состояния при закрытии позиции (перед новым входом)
    if (this.position !== 0 && (longCond || shortCond)) {
      resetTradeState()
    }

    // ------------------------------ ENTRY ----------------------------
    if (longCond && this.position <= 0) {
      this.tradeId++
      resetTradeState()
    ...
    if (this.position > 0 && this.positionSizePercentCurrent > 0) {
      ...
      // TP1 (25%)
      if (!this.longReachedTp1 && high >= this.longTp1Price) {
        ...
        if (slHit && tp1Hit) {
          ...
          } else if (fill.type === "limit" && !this.longTp1Executed) {
            // TP достигнут первым
      ...
      // TP2 (33%)
      if (this.longReachedTp1 && !this.longReachedTp2 && !this.longTp2Executed && high >= this.longTp2Price) {
    ...
    if (this.position < 0 && this.positionSizePercentCurrent > 0) {
      ...
      // TP1 (25%)
      if (!this.shortReachedTp1 && low <= this.shortTp1Price) {
        ...
        if (slHit && tp1Hit) {
          ...
          } else if (fill.type === "limit" && !this.shortTp1Executed) {
            // TP достигнут первым
      ...
      // TP2 (33%)
      if (this.shortReachedTp1 && !this.shortReachedTp2 && !this.shortTp2Executed && low <= this.shortTp2Price) {
        // Определяем текущий SL (может быть уже пересоздан после TP1)
